...
class UnitsTest extends FunSuite {
    test("ChunkUnit has correct bounds") {
        ...
        assert(0.chunk.upper === 15.cube)
        assert(1.chunk.lower === 16.cube)
        assert(1.chunk.upper === 31.cube)
        assert(-1.chunk.upper === -1.cube)
        assert(-1.chunk.lower === -16.cube)
        assert(-2.chunk.lower === -32.cube)
        assert(-2.chunk.upper === -17.cube)
        assert(13.chunk.lower === 208.cube)
        assert(13.chunk.upper === 223.cube)

        assert(0.cube.bound === 0.chunk)
        assert(5.cube.bound === 0.chunk)
        assert(15.cube.bound === 0.chunk)
        assert(16.cube.bound === 1.chunk)
        assert(18.cube.bound === 1.chunk)
        assert(-1.cube.bound === -1.chunk)
        assert(-16.cube.bound === -1.chunk)
        assert(-17.cube.bound === -2.chunk)
        assert(-32.cube.bound === -2.chunk)
    }
